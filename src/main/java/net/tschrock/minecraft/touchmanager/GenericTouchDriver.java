  ...
  implements ITouchDriver
{
  ...
  protected void onNewEvent(TouchEvent event)
  {
    ...
    if (event.touchType == TouchEvent.Type.TOUCH_START) {
      DebugHelper.log(DebugHelper.LogLevel.DEBUG1, "Got 'TOUCH_START' at (" + event.getX() + ", " + event.getY() + ") with id=" + event.getId());
      this.currentTouchState.add(event);
    }
    else if (event.touchType == TouchEvent.Type.TOUCH_UPDATE) {
      DebugHelper.log(DebugHelper.LogLevel.DEBUG2, "Got 'TOUCH_UPDATE' at (" + event.getX() + ", " + event.getY() + ") with id=" + event.getId());
      int index = findIndexByTouchId(event.touchId);
      if (index != -1) {
        this.currentTouchState.set(index, event);
      }
      else {
        DebugHelper.log(DebugHelper.LogLevel.WARNING, "'TOUCH_UPDATE' with id=" + event.getId() + " has no matching start event!");
      }
    }
    else if (event.touchType == TouchEvent.Type.TOUCH_END) {
      DebugHelper.log(DebugHelper.LogLevel.DEBUG1, "Got 'TOUCH_END' at (" + event.getX() + ", " + event.getY() + ") with id=" + event.getId());
      int index = findIndexByTouchId(event.touchId);
      if (index != -1) {
        this.currentTouchState.remove(index);
      }
      else {
        DebugHelper.log(DebugHelper.LogLevel.WARNING, "'TOUCH_END' with id=" + event.getId() + " has no matching start event!");
      }
    }
  }
