...
public class MCGuiTouchJoystick extends MCGuiTouchComponent {
    ...
    public void onTouchEvent(TouchEvent event) {
        if ((event.getType() == net.tschrock.minecraft.touchmanager.TouchEvent.Type.TOUCH_START)
                && (this.boundTouch == null)) {
            this.boundTouch = event;
        }

        if ((event.getType() == net.tschrock.minecraft.touchmanager.TouchEvent.Type.TOUCH_UPDATE)
                && (this.boundTouch != null) && (this.boundTouch.getId() == event.getId())) {
            this.boundTouch = event;
        }

        if ((event.getType() == net.tschrock.minecraft.touchmanager.TouchEvent.Type.TOUCH_END)
                && (this.boundTouch != null) && (this.boundTouch.getId() == event.getId())) {
            this.boundTouch = null;
        }
    }
    ...
    protected void calculateJoystickPosition() {
        if (this.boundTouch == null) {
            int centeredTouchX = this.boundTouch.getAdjustedX(this.parentScreen.width) - this.preferedX
                    - (this.preferedX + this.preferedWidth / 2);
            int j = this.boundTouch.getAdjustedY(this.parentScreen.height) - this.preferedY
                    - (this.preferedY + this.preferedHeight / 2);
        } else {
            int y;
            int i = y = 0;
        }
